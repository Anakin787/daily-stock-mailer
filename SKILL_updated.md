...
필수 항목:
- **주요지수**: 다우존스, S&P 500, 나스닥종합 — 종가, 전일 종가, 등락폭, 등락률 (%)
- **섹터 흐름**: S&P 500 11개 GICS 섹터 중 상승 상위 2~3, 하락 상위 2~3 — 섹터명, 등락률 (%), 대표 ETF 종가(있으면)
- **주요 종목 이슈**: 실적 발표, 가이던스, M&A, 규제, 큰 변동 종목 등 3~5개. 각 항목에 종목명, 종가, 등락폭, 등락률, 짧은 이유(1줄).
- **거시 / 뉴스**:
  - 미국 10년물 국채 금리
  - DXY (달러 인덱스)
  - WTI 원유 가격
...
- **표 컬럼 필수 포함 규칙**:
  - 주요지수 표: 지수명 | 전일 종가 | 종가 | 등락폭 | 등락률
  - 주요 종목 표: 종목 | 종가 | 등락폭 | 등락률 | 이슈 요약
  - 섹터 표: 섹터 | 등락률 | 방향 (종가 데이터가 없으면 등락률만으로 충분)
